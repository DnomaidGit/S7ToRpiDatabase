...
public abstract class S7Connection {
    ...
    public int readBytes(
        ...
        int res = 0;
        semaphore.enter();
        mensPanel.MnsText("PLC_S7: Reading bytes...");
        ...
        if (res != Nodave.RESULT_OK) {
            semaphore.leave();
            mensPanel.MnsText("PLC_S7: Read bytes result NOK mensOut");
            return res;
        }
        ...
        if ((Nodave.Debug & Nodave.DEBUG_CONN) != 0) {
            mensPanel.MnsText("PLC_S7: SetupReceivedPDU() returned: " + res + Nodave.strerror(res));
        }
        if (res != Nodave.RESULT_OK) {
            semaphore.leave();
            mensPanel.MnsText("PLC_S7: Read bytes result NOK mensIn1");
            return res;
        }    
        
        res = p2.testReadResult();
        if ((Nodave.Debug & Nodave.DEBUG_CONN) != 0) {
            mensPanel.MnsText("PLC_S7: TestReadResult() returned: " + res + Nodave.strerror(res));
        }
        if (res != Nodave.RESULT_OK) {
            semaphore.leave();
            mensPanel.MnsText("PLC_S7: Read bytes result NOK mensIn2");            
            return res;
        }
        
        if (p2.udlen == 0) {
            semaphore.leave();
            mensPanel.MnsText("PLC_S7: Result read bytes result = 0");        
    ...
    public int writeBytes(
        ...
        int errorState = 0;
        semaphore.enter();
        mensPanel.MnsText("PLC_S7: Writing bytes...");
...
//		p1.constructWriteRequest(area, DBnum, start, len, buffer);
        ...
        if (errorState == 0) {
            ...
            if (p2.mem[p2.param + 0] == PDU.FUNC_WRITE) {
                if (p2.mem[p2.data + 0] == (byte) 0xFF) {
                    if ((Nodave.Debug & Nodave.DEBUG_CONN) != 0) {
                        mensPanel.MnsText("PLC_S7: writeBytes success");
                    }
                    semaphore.leave();
                    return 0;
                }
                ...
                errorState |= 4096;
                mensPanel.MnsText("PLC_S7: Write bytes result NOK mensOut");
            }
        }
        semaphore.leave();
        return errorState;
        ...
        public synchronized void enter() {
            ...
            if (value < 0) {
                try {
                    ...
                    mensPanel.MnsException("PLC_S7",e);
                }
            }
        }
        ...
        if (p2.mem[p2.param + 0] == PDU.FUNC_READ) {
            ...
            for (int i = 0; i < numResults; i++) {
                ...
                if (r.error == 255) {
                    ...
                    mensPanel.MnsText("PLC_S7: Error " + r.error);
                }
                pos += 4;
                rs.results[i] = r;
            }
    ...
    public int useResult(ResultSet rs, int number) {
        mensPanel.MnsText("PLC_S7: rs.getNumResults: " + rs.getNumResults() + " number: " + number);
	...
	*/
	public int negPDUlengthRequest() {
        ...
        if ((Nodave.Debug & Nodave.DEBUG_CONNECT) != 0) {
            mensPanel.MnsText("PLC_S7: Partner offered PDU length: " + maxPDUlength);                    
        }
        return res;
    }
    /*
    ...
    public void finalize() throws Throwable {
        mensPanel.MnsText("PLC_S7: This is finalize");
        disconnectPLC();
        iface.finalize();
    }
}
